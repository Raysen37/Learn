...
static void SaveLEDState(uint8_t ledState)
{
  ...
  HAL_SPI_Transmit(&hspi1, sectorErase, sizeof(sectorErase) / sizeof(sectorErase[0]), HAL_MAX_DELAY);
  ...
  HAL_SPI_Transmit(&hspi1, pageProgCmd, sizeof(pageProgCmd) / sizeof(pageProgCmd[0]), HAL_MAX_DELAY);
...
static uint8_t LoadLEDState(void)
{
  ...
  HAL_SPI_Transmit(&hspi1, readDataCmd, sizeof(readDataCmd) / sizeof(readDataCmd[0]), HAL_MAX_DELAY);
...
int main(void)
{
  ...
  uint8_t ledState = 0;
	...
	HAL_UART_Transmit(&huart1, &ledState, 1, HAL_MAX_DELAY);
